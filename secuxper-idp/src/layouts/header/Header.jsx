...
const Header = ({ id }) => {
  ...
  // Add click event listener to document and remove it when component unmounts
  // The reason we do this is that the header is a fixed element and does not have a click event
  // listener on its own, so we need to listen for click events on the entire document
  // and check if the target is inside the header component
  useEffect(() => {
  ...
  // Toggle favourites and functions menu when favourites icon is clicked
  // Update favourites icon with "active" class
  useEffect(() => {
    const faStar = document.getElementById("favourite-icon");
    if (showFavourites) {
      setShowAllFunctions(false);
      faStar.classList.add("star-active"); // Add "active" class to the favourites icon when the menu is shown
    } else {
      faStar.classList.remove("star-active"); // Remove "active" class from the favourites icon when the menu is hidden
    }
  }, [showFavourites]);

  // Hide favourites menu when functions menu is shown
  useEffect(() => {
    if (showAllFunctions) {
      setShowFavourites(false); // Hide favourites menu when functions menu is shown
    }
  }, [showAllFunctions]);

  // Hide notifications when profile options menu is shown
  useEffect(() => {
    if (showProfileOptions) {
      setShowNotification(false); // Hide notifications when profile options menu is shown
    }
  }, [showProfileOptions]);

  // Hide profile options menu when notifications are shown
  useEffect(() => {
    if (showNotification) {
      setShowProfileOptions(false); // Hide profile options menu when notifications are shown
    }
  }, [showNotification]);
  ...
  const handleSearchChange = (e) => {
    ...
    const filteredItems = childrenItems
      ...
      .slice(0, 20); // Only show the first 20 search results

    // Update suggestions
    const formattedSuggestions = filteredItems.map((child) => {
      const titleParts = child.title.split(
        new RegExp(`(${value})`, "gi") // case-insensitive (i) and global search (g)
      );
  ...
  const handleSearchFocus = () => {
    ...
    searchIcon.classList.add("highlight"); // Add "highlight" class to the search icon when input is focused
  ...
  const handleSearchBlur = () => {
    const searchIcon = document.getElementById("search-icon");
    searchIcon.classList.remove("highlight"); // Remove "highlight" class from the search icon when input is blurred
  };
        ...
        <div className="d-flex">
          <div className="d-flex" ref={favouriteAndMenuRef}>
            ...
            <div
              ...
              onClick={() => setShowAllFunctions(prevState => !prevState)}
            >
